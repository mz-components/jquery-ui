...
(function (factory) {
    ...
    return $.effects.define('fold', 'hide', function (options, done) {
        ...
        var element = $(this), mode = options.mode, show = mode === 'show', hide = mode === 'hide', size = options.size || 15, percent = /([0-9]+)%/.exec(size), horizFirst = !!options.horizFirst, ref = horizFirst ? [
                ...
                'bottom',
                'right'
            ], duration = options.duration / 2, placeholder = $.effects.createPlaceholder(element), start = element.cssClip(), animation1 = { clip: $.extend({}, start) }, animation2 = { clip: $.extend({}, start) }, distance = [
                start[ref[0]],
                start[ref[1]]
